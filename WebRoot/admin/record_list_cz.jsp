...
<div class="page-content clearfix">
    <div id="Member_Ratings">
      ...
      <form method="post" action="recordList_cz_search.do" >
    <div class="search_style">
      <ul class="search_content clearfix">
       <li>
        <li><label class="l_f">会员：</label>
        <select name="gid"  class="text_add"  style=" width:400px">
        <option value="">请选择会员</option>
        <c:forEach items="${glist }" var="g">
        <option value="${g.id }">${g.uname }(${g.idcard })</option>
        </c:forEach>
        </select>
       </li>
     ...
     <div class="border clearfix">
      <!--  <span class="l_f">
       <a href="user_add_hy.jsp" id="member_add" class="btn btn-warning"><i class="icon-plus"></i>添加用户</a>
       </span> -->
     </div>
	...
	<tbody>
		...
		<tr>
          <td><c:forEach items="${ulist }" var="y">
          <c:if test="${y.id==u.gid }">${y.uname }</c:if>
          </c:forEach></td>
          <td><c:forEach items="${ulist }" var="y">
          <c:if test="${y.id==u.gid }">${y.tname }</c:if>
          </c:forEach></td>
            <td><c:forEach items="${ulist }" var="y">
          <c:if test="${y.id==u.gid }">${y.idcard }</c:if>
          </c:forEach></td>
          <%-- <td> <img alt="" src=".<s:property value="img" />" width="50" height="50"></td> --%>
          <td>${u.price}</td>
          <td>${u.stime }</td>
          <td class="td-manage">
          <a title="编辑" href="doUpdateRecord_cz.do?id=${u.id }"  class="btn btn-xs btn-info" ><i class="icon-edit bigger-120">修改</i></a>
          <a title="删除" href="deleteRecord_cz.do?id=${u.id }"  onclick="return del(1,1,1)" class="btn btn-xs btn-warning" ><i class="icon-trash  bigger-120">删除</i></a>
          </td>
        </tr>
        </c:forEach>
      <tr>
		...
		<td colspan="8"> <c:if test="${page.page>1}">
              <a href="recordList_cz.do?page=1" >首页</a>
             <a href="recordList_cz.do?page=${page.page-1 }"> 上一页</a> 
             </c:if>
    	     <c:if test="${page.page<page.totalPage}">
			<a href="recordList_cz.do?page=${page.page+1 }">下一页</a>
			<a href="recordList_cz.do?page=${page.totalPage }">末页</a>
		    </c:if> 
		</td>
	</c:if>
	<c:if test="${sessionScope.p==2 }">
	<td colspan="8"> <c:if test="${page.page>1}">
    <a href="recordList_cz_search.do?page=1" >首页</a>
    <a href="recordList_cz_search.do?page=${page.page-1 }"> 上一页</a> 
    </c:if>
    	<c:if test="${page.page<page.totalPage}">
			<a href="recordList_cz_search.do?page=${page.page+1 }">下一页</a>
			<a href="recordList_cz_search.do?page=${page.totalPage }">末页</a>
		</c:if> 
		</td>
		</c:if>
		</tr>
