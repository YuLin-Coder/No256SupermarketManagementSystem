$(function(){
	...
	$pintuercheck=function(element,type,value){
		...
		switch(type){
			...
			case "csglxt":return /^((\(\d{2,3}\))|(\d{3}\-))?(\(0\d{2,3}\)|0\d{2,3}-)?[1-9]\d{6,7}(\-\d{1,4})?$/.test($pintu);break;
	...
	$('.banner').each(function(){
		...
		var csglxts=e.attr("data-csglxt");
		var csglxts_s=e.attr("data-small");
		var csglxts_m=e.attr("data-middle");
		var csglxts_b=e.attr("data-big");
		var num=e.find(".carousel .csglxt").length;
		var win=$(window).width();
		var i=1;

		if(interval==null){interval=5};
		if(csglxts==null || csglxts<1){csglxts=1};
		if(csglxts_s!=null && win>760){csglxts=csglxts_s};
		if(csglxts_m!=null && win>1000){csglxts=csglxts_m};
		if(csglxts_b!=null && win>1200){csglxts=csglxts_b};

		var csglxtWidth=Math.ceil(e.outerWidth()/csglxts);
		var page=Math.ceil(num/csglxts);
		e.find(".carousel .csglxt").css("width",csglxtWidth+ "px");
		e.find(".carousel").css("width",csglxtWidth*num + "px");
		
		var carousel=function(){
			i++;
			if(i>page){i=1;}
			$showbanner(e,i,csglxts,num);
		};
		...
		if(pointer!=0 && page>1){
			...
			pager.find("li").click(function(){
				$showbanner(e,$(this).val(),csglxts,num);
			});
	...
	$showbanner=function(e,i,csglxts,num){
		var after=0,leftx=0;
		leftx = - Math.ceil(e.outerWidth()/csglxts)*(csglxts)*(i-1);
		if(i*csglxts > num){after=i*csglxts-num;leftx= - Math.ceil(e.outerWidth()/csglxts)*(num-csglxts);};
