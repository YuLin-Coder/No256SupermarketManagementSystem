...
function checkName($txtObj){
	 var len=$txtObj.val().length;
 if(len==0){
   $txtObj.siblings("span").text("预留问题必填");
   return false;
 }
 /*if(len<2||len>6){
...
function checkcsglxt($txtObj){
	 var csglxtReg=/^(13|15|18)\d{9}$/;
	 var len=$txtObj.val().length;
	if(len==0){
   $txtObj.siblings("span").text("手机号必填");
   return false;
 }
 if(!csglxtReg.test($txtObj.val())){
...
function check(){
	var flag= checkNo($("#username"))
	      &&checkPwd($("#txtPwd"))
		  &&checkConfirm($("#txtConfirmPwd"))
		  &&checkName($("#txtName"))
		  //&&checkId($("#txtId"))
		  && checkcsglxt($("#txtcsglxt"));
	return flag;
}
//文本框获取焦点时的样式
...
function loseFocus($txtObj){
 ...
 $(function(){
	...
	$("#username").focus(function(){//绑定账号文本框获取焦点事件
	  getFocus($(this));
	});
	$("#username").blur(function(){//绑定账号文本框丢失焦点事件
	   loseFocus($(this)); //改样式
	   checkNo($(this));//判断对错
	});
	...
	$("#txtcsglxt").focus(function(){//绑定手机号码文本框获取焦点事件
	  getFocus($(this));
	});
	$("#txtcsglxt").blur(function(){//绑定手机号码文本框丢失焦点事件
	 loseFocus($(this)); //改样式
	 checkcsglxt($(this));//判断对错
	});		
});