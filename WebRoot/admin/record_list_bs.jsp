...
<div class="page-content clearfix">
    <div id="Member_Ratings">
      ...
      <form method="post" action="recordList_bs_search.do" >
    <div class="search_style">
      ...
      <!--   <li><label class="l_f">商品名</label><input name="name" type="text"  class="text_add" placeholder="请输入搜索商品名"  style=" width:400px"/></li> -->
        <li><select name="gid"class="text_add" style="width:80px;" >
     ...
     <!--   <span class="l_f">
       <a href="doAddRecord.do" id="member_add" class="btn btn-warning"><i class="icon-plus"></i>添加补货</a>
       </span> -->
     </div>
	...
	<tbody>
		...
		<tr>
           <td><c:forEach items="${glist }" var="ad">
          <c:if test="${jz.gid==ad.id }">
          ${ad.name}</c:if></c:forEach></td>
          <td>${jz.cname}</td>
        <%--   <td>${jz.price}</td> --%>
          <td>${jz.snum }</td>
          <td>${jz.stime }</td>
          <td class="td-manage">
          <a title="编辑" href="doUpdateRecord_bs.do?id=${jz.id }"  class="btn btn-xs btn-info" ><i class="icon-edit bigger-120">修改</i></a>
          <a title="删除" href="deleteRecord_bs.do?id=${jz.id }"  onclick="return del(1,1,1)" class="btn btn-xs btn-warning" ><i class="icon-trash  bigger-120">删除</i></a>
          </td>
        </tr>
        </c:forEach>
      <tr>
		...
		<td colspan="8"> <c:if test="${page.page>1}">
              <a href="recordList_bs.do?page=1" >首页</a>
             <a href="recordList_bs.do?page=${page.page-1 }"> 上一页</a> 
             </c:if>
    	     <c:if test="${page.page<page.totalPage}">
			<a href="recordList_bs.do?page=${page.page+1 }">下一页</a>
			<a href="recordList_bs.do?page=${page.totalPage }">末页</a>
		    </c:if> 
		</td>
	</c:if>
	<c:if test="${sessionScope.p==2 }">
	<td colspan="8"> <c:if test="${page.page>1}">
    <a href="recordList_bs_search.do?page=1" >首页</a>
    <a href="recordList_bs_search.do?page=${page.page-1 }"> 上一页</a> 
    </c:if>
    	<c:if test="${page.page<page.totalPage}">
			<a href="recordList_bs_search.do?page=${page.page+1 }">下一页</a>
			<a href="recordList_bs_search.do?page=${page.totalPage }">末页</a>
		</c:if> 
		</td>
		</c:if>
		</tr>
